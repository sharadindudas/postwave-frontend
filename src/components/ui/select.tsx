  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        ...
        "w-[150px] h-12! bg-cc-neutral-100 text-cc-primary-2",
        className,
      )}
      {...props}
    >
...
function SelectItemWithDescription({
  ...
  return (
    <SelectItem value={value} {...props} className={cn("focus:bg-cc-background-4", className)}>
      <div
        className={cn("flex flex-col item-start gap-1 text-left w-full text-sm", contentClassName)}
      >
        <p className="font-semibold text-cc-primary-2-600">{name}</p>
        <p className="text-cc-primary-2-500 text-xs">{description}</p>
      </div>
    </SelectItem>
  );
}
